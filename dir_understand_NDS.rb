# OUTPUTS location coordinants for all data
...
puts "This NDS is an array re: DIRECTORS AND STATS ON THEIR MOVIES"
row = 0
while row < nds.length do
	puts "ROW #{row} has DIRECTOR #{nds[row][:name]}'s stats and "
	puts " each Hash has 2 keys => :name & :movies"
	puts "#{nds[row]} "
	
	puts "\t Coordinant: [#{row}, :movies] points to director #{nds[row][:name]}'s whole movie #{nds[row][:movies].class} containing #{nds[row][:movies].length} hashes."
	puts "NOTE Below add a key to the coordinants to get the value."

	dir_row = 0
	while dir_row < nds[row][:movies].length do

		puts "\t\t (#{row}, :movies, #{dir_row}) is: #{nds[row][:movies][dir_row]}"
		dir_row += 1
	end
